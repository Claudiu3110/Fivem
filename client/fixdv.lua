    ...
    if(DoesEntityExist(ped) and not IsEntityDead(ped))then 
        ...
        if(IsPedSittingInAnyVehicle(ped))then 
            ...
            if(GetPedInVehicleSeat(vehicle, -1) == ped)then 
                ...
                if(DoesEntityExist(vehicle))then 
                	tvRP.notify("Masina nu s-a putut sterge, Incearca din nou!")
                else 
                	tvRP.notify("Vehiculul s-a sters!")
                end
            else 
                tvRP.notify("Trebuie sa fi soferul vehiculului!")
            end
        else
            ...
            if(DoesEntityExist(vehicle))then
                ...
                if(DoesEntityExist(vehicle))then
                	...
                	tvRP.notify("Vehiculul este sters!")
                end
            else
                tvRP.notify("Trebuie sa fi soferul vehiculului!")
            end
...
RegisterNetEvent("claudiu:fix")
AddEventHandler("claudiu:fix", function()
	local playerPed = PlayerPedId()
	if IsPedInAnyVehicle(playerPed) then
		local vehicle = GetVehiclePedIsIn(playerPed)
		SetVehicleEngineHealth(vehicle, 9999)
